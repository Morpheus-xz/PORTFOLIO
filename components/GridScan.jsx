...
export const GridScan = ({
  ...
  const perfRef = useRef({
    isLowPower:
      (typeof navigator !== 'undefined' && ((navigator.deviceMemory && navigator.deviceMemory <= 4) || navigator.hardwareConcurrency <= 4)) ||
      false
  });
  const inViewRef = useRef(true);

  const [modelsReady, setModelsReady] = useState(!enableWebcam);
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({
      antialias: false,
      alpha: true,
      powerPreference: perfRef.current.isLowPower ? 'low-power' : 'high-performance'
    });
    ...
    const tick = () => {
      ...
      const frameBudget = inViewRef.current ? minFrameTimeVisible : minFrameTimeOffscreen;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        await Promise.all([
          mod.nets.tinyFaceDetector.loadFromUri(modelsPath),
          mod.nets.faceLandmark68TinyNet.loadFromUri(modelsPath)
        ]);
