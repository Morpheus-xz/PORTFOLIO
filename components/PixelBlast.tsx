...
void main(){
  ...
  fragColor = vec4(srgbColor, M * 0.4); // Made it softer matching aesthetics!
}
`;
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const canvas = document.createElement('canvas');
      canvas.style.pointerEvents = 'none'; // allow clicks to pass through if needed, but let's keep interactions since we want ripples.
      ...
      // High-DPI screens quadruple the amount of shaded pixels. For a background effect, 
      // running at DPR=1 maintains aesthetics but provides huge performance uplifts.
